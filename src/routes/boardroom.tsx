...
function BoardroomPage() {
  ...
  // Phase 11, Live AI debate: real Google Gemini reasoning for every board agent
  ...
  // Phase 10, Real CEO execution via Google Gemini (server function).
  ...
  // existing Section 14 / 15 UI surfaces live Google Gemini reasoning for CEO.
  ...
  // When the live Google Gemini debate has produced replies, the headline Board
          ...
          <Section icon={<Trophy className="h-4 w-4" />} label="06" title="Board Decision" subtitle="The synthesized executive decision.">
            ...
            <div className="executive-card-elevated rounded-xl p-6">
              ...
              <p className="text-[10px] text-muted-foreground mt-3">
                {hasLiveDebate ? "Consensus & confidence computed from the live Google Gemini agent debate." : "Heuristic baseline. Start a meeting to run the live AI debate."}
            ...
            <div className="executive-card rounded-xl p-5 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-1">
                ...
                {debateBusy ? (
                  ...
                  <span className="text-xs text-muted-foreground">Run a live debate to have each agent reason with real AI (Google Gemini).</span>
                )}
              </div>
            </div>
          </Section>
...
function ContextPreview({ context, scenario }: { context: ExecutiveContext; scenario: SimulationScenario | null }) {
  ...
  return (
    <Section icon={<Cpu className="h-4 w-4" />} label="01.6" title="Executive Context Preview" subtitle="The full briefing object the agents reason from, the same payload sent to Google Gemini on each live debate.">
...
function DecisionFramework({ record }: { record: BoardDecisionRecord }) {
  return (
    <Section icon={<ShieldCheck className="h-4 w-4" />} label="06.5" title="Board Decision Framework" subtitle="The canonical decision schema each Google Gemini agent response is validated against.">
...
function BrainStatus({ readiness }: { readiness: BrainReadiness }) {
  ...
  return (
    <Section icon={<Brain className="h-4 w-4" />} label="17" title="Executive Brain Status" subtitle="Reasoning stack powering the live Google Gemini agents. Each component is wired, inspectable, and in use during a live debate.">
      <div className="grid lg:grid-cols-[260px_1fr] gap-4">
        <div className="executive-card-elevated rounded-xl p-6 flex flex-col items-center">
          <ScoreRing value={readiness.score} label="AI Readiness" size={140} tone={tone} />
          <p className="text-[11px] text-muted-foreground text-center mt-3">
            {readiness.score >= 80 ? "Reasoning stack fully wired and running live against Google Gemini." : readiness.score >= 50 ? "Most components wired, finish the partials to unlock full reasoning." : "Foundation in place, load a dataset and seed memory to advance."}
          </p>
        </div>
...
function OrchestrationCenter({ status }: { status: ReturnType<typeof orchestrationStatus> }) {
  return (
    <Section icon={<Workflow className="h-4 w-4" />} label="11" title="LLM Orchestration Center" subtitle="The pipeline ExecutiveOS uses to talk to Google Gemini. Every stage is wired, inspectable, and runs on each live debate.">
...
function PromptBuilderInspector({ bundle }: { bundle: ExecutivePromptBundle }) {
  return (
    <Section icon={<Terminal className="h-4 w-4" />} label="12" title="Executive Prompt Builder" subtitle="The exact prompt and context payload sent to Google Gemini on each live debate. Shown here for inspection.">
...
function AgentPromptGeneration({ prompts }: { prompts: AgentPromptObject[] }) {
  return (
    <Section icon={<Layers className="h-4 w-4" />} label="13" title="Agent Prompt Generation" subtitle="Per-agent prompt objects produced for every persona and sent to Google Gemini during a live debate.">
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
        {prompts.map((p) => (
...
function AgentContractsPanel() {
  ...
  return (
    <Section icon={<ShieldCheck className="h-4 w-4" />} label="13.5" title="Agent Contracts" subtitle="Production-grade reasoning contracts. Each persona binds decision rules, must-consider fields, allowed stances, and a strict response schema enforced on every Google Gemini response.">
...
function AgentExecutionPanel({
  ...
  return (
    <Section
      ...
      subtitle="Phase 10 · CEO agent executes against Google Gemini. Remaining agents run on the local heuristic engine until they are upgraded."
    >
      ...
      detail:
        ceoExecution.result && ceoExecution.result.ok
          ...
          : ceoExecution.result && !ceoExecution.result.ok
            ...
            : ceoExecution.state === "running"
              ? "Dispatching CEO prompt to Google Gemini…"
...
function ProviderReadinessPanel({
  ...
  return (
    <Section icon={<Plug className="h-4 w-4" />} label="16" title="Provider Readiness" subtitle="The LLM provider ExecutiveOS runs against. The board agents execute against Google Gemini in real time via server-side AWS credentials.">
      <div className="grid md:grid-cols-3 gap-3 mb-3">
        {PROVIDERS.map((p) => {
          ...
          const notes = isGemini
            ? connected
              ? "Live · executing real CEO agent via server-side the Gemini API key."
              : "the Gemini API key/model access not configured on the server. CEO falls back to local heuristic engine."
            : p.notes;
          return (
