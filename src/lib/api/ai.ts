    ...
    if (!res.ok) {
      brainNote =
        res.error.code === "rate_limit"
          ? "> ⚠️ _Live AI is rate-limited, the Google Gemini request quota for this account is exhausted (try again shortly, or request a quota increase). Showing built-in analysis until it recovers._\n\n"
          : res.error.code === "missing_key"
            ? "> ⚠️ _Live AI isn't configured, the Gemini API key are missing or model access isn't enabled on the server. Showing built-in analysis._\n\n"
